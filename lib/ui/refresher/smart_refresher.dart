...
import 'indicator/ym_indicator.dart';
...
enum RefreshStatus {
  ...
  /// the indicator is refreshing but not call onRefresh,waiting for the finish callback
  refreshing_without_notify,
...
class SmartRefresherState extends State<SmartRefresher> {
  ...
  final LoadIndicator defaultFooter = YmFooter();
  ...
  Widget _buildBodyBySlivers(Widget childView, List<Widget> slivers, RefreshConfiguration conf) {
    ...
    if (childView is! Scrollable) {
      ...
      if (childView is ScrollView) {
        ...
        widget.controller.scrollController =
            scrollController ?? childView.controller ?? (childView.primary ? PrimaryScrollController.of(context) : null);
      } else {
        // ignore: DEPRECATED_MEMBER_USE_FROM_SAME_PACKAGE
    ...
    /*try {
      throw '${childView.runtimeType} , ${body.runtimeType}';
    } catch (e, stack) {
      print(e);
      print(stack);
    }*/
  ...
  void initState() {
    if (widget.controller.initialRefresh) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        if (mounted) widget.controller.requestRefresh(notify: widget.controller.initialNotify);
      });
    }
    super.initState();
  }
...
class RefreshController {
  ...
  RefreshController(
      {this.initialRefresh: false, this.initialNotify: false, RefreshStatus initialRefreshStatus, LoadStatus initialLoadStatus}) {
  ...
  Future<void> requestRefresh({
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      return Future.delayed(const Duration(milliseconds: 50)).then((_) async {
        await position?.animateTo(position.minScrollExtent, duration: duration, curve: curve)?.then((_) {
          SmartRefresher.ofState(position.context.storageContext)?.setCanDrag(true);
          headerMode.value = notify ? RefreshStatus.refreshing : RefreshStatus.refreshing_without_notify;
        });
      });
    });
    return Future.value();
